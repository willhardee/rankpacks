        ...
        <div className="mx-auto max-w-4xl px-4 py-3">
          <nav aria-label="Primary tabs" className="grid grid-cols-3 rounded-xl bg-neutral-100 p-1 dark:bg-neutral-800">
            {tabs.map((tab) => {
              ...
              const Icon = tab.icon;
              return (
                <Link key={tab.href} className={cn('inline-flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition', active ? 'bg-white text-neutral-900 shadow-sm dark:bg-neutral-700 dark:text-white' : 'text-neutral-700 hover:bg-white dark:text-neutral-300 dark:hover:bg-neutral-700')} href={tab.href}>
